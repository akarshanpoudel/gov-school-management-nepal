...
class Subject(models.Model):
    ...
    # CDC Minimums (35% for theory, 40% for practical)
...
class Mark(models.Model):
    ...
    class Meta:
        # A student can only have one mark entry per subject per exam
        unique_together = ('student', 'exam', 'subject')

    @property
    def is_ng(self):
        """Calculates if the student gets an NG (Not Graded) based on CDC rules."""
    ...
    def grade_point(self):
        """Calculates Grade Point based on total percentage ONLY if not NG."""
        if self.is_ng:
            return 0.0
